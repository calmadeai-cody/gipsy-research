...
import { generateResearchGaps } from '@/lib/ai/research-gap-generator'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { research_topic, research_timeline } = await request.json()
    ...
    const sanitizedTimeline = research_timeline ? sanitizeInput(research_timeline, MAX_TIMELINE_LENGTH) : undefined
    ...
    const toolName = 'research-gap-generator'
    ...
    // Check cache first
    const cacheKey = generateCacheKey(toolName, sanitizedTopic + (sanitizedTimeline ? ':' + sanitizedTimeline : ''))
    ...
    const result = await generateResearchGaps(sanitizedTopic, sanitizedTimeline)
    ...
    console.error('Research Gap Generator error:', error)
    return NextResponse.json(new ApiError('Failed to generate research gaps', ErrorCodes.INTERNAL_ERROR, 500).toJSON(), { status: 500 })
  }
}